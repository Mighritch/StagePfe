package com.bea.bea_bi_backend.repositories.analytics;

import com.bea.bea_bi_backend.entities.analytics.VwAbsencesParMotif;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.stereotype.Repository;
import java.util.List;

@Repository
public interface VwAbsencesParMotifRepository
        extends JpaRepository<VwAbsencesParMotif, String> {

    @Query("SELECT v FROM VwAbsencesParMotif v " +
            "WHERE (:service IS NULL OR v.libServ = :service) " +
            "AND (:annee IS NULL OR v.annee = :annee) " +
            "AND (:motif IS NULL OR v.libMot = :motif)")
    List<VwAbsencesParMotif> findByFiltres(
            @Param("service") String service,
            @Param("annee") Integer annee,
            @Param("motif") String motif);
}